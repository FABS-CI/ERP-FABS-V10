...
def build_articles_rows(articles: List[Dict]) -> str:
    """Génère les lignes HTML du tableau d'articles"""
    rows = []
    for article in articles:
        row = f"""
        <tr>
            <td>{article.get('classe', '')}</td>
            <td>{str(article.get('code_article') or article.get('produit_id', ''))[:14]}</td>
            <td>{article.get('designation', '')}</td>
            <td>{article.get('quantite', 0)}</td>
            <td>{format_fcfa(float(article.get('prix_unitaire', 0)))}</td>
            <td>{format_fcfa(float(article.get('montant_ht', 0)))}</td>
        </tr>
        """
        rows.append(row)
    return "".join(rows)
