...
def project_product(doc: dict, *, see_prix_achat: bool) -> dict:
    ...
    # Compat : `code_article` (legacy seed) → `reference`
    ...
    async def update_product(
        ...
        if log_audit_event:
            await log_audit_event(
                ...
                details={
                    "reference": old_product.get("reference"),
                    "titre": old_product.get("titre"),
                    "old_values": {k: old_product.get(k) for k in updates.keys()},
                    "new_values": updates
                },
                ip_address=request.client.host if request.client else None
            )
        ...
        if log_audit_event:
            await log_audit_event(
                ...
                details={
                    "reference": old_product.get("reference"),
                    "titre": old_product.get("titre"),
                    "old_actif": old_product.get("actif"),
                    "new_actif": False
                },
                ip_address=request.client.host if request.client else None
            )
