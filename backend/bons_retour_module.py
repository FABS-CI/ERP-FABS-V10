...
def build_bons_retour_router(db: AsyncIOMotorDatabase, resolve_user, log_audit_event=None) -> APIRouter:
    ...
    async def list_bons_retour(
        ...
        pipeline = [
            ...
            {"$project": {"client_info": 0, "facture_info": 0, "_id": 0}},
        ]
        if q:
        ...
        pipeline += [{"$sort": {"date_retour": -1}}, {"$limit": limit}]

        docs = await db.bons_retour.aggregate(pipeline).to_list(limit)
        ...
        if log_audit_event:
            await log_audit_event(
                ...
                details={
                    ...
                    "facture_id": br["facture_id"],
                    "facture_reference": facture.get("reference") if 'facture' in locals() else None,
                    "avoir_id": avoir_id,
                    "avoir_reference": avoir_reference,
                    "montant_total_ht": br["montant_total_ht"],
                    "montant_total_ttc": br["montant_total_ttc"],
                    "lignes_count": len(br_lignes),
                    "mouvements_stock_count": len(br_lignes)
                },
                ip_address=request.client.host if request.client else None
            )
