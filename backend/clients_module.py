...
def build_clients_router(
    ...
    async def list_clients(
        ...
        if q:
            ...
            filt["$or"] = [
                ...
                {"representant": {"$regex": esc, "$options": "i"}},
            ]
    ...
    async def update_client(
        ...
        _ensure(old_client is not None, 404, "Client introuvable")
        ...
        if log_audit_event:
            await log_audit_event(
                ...
                details={
                    "reference": old_client.get("reference"),
                    "old_values": {k: old_client.get(k) for k in updates.keys()},
                    "new_values": updates
                },
                ip_address=request.client.host if request.client else None
            )
    ...
    async def disable_client(
        ...
        if log_audit_event:
            await log_audit_event(
                ...
                details={
                    "reference": old_client.get("reference"),
                    "nom": old_client.get("nom"),
                    "old_actif": old_client.get("actif"),
                    "new_actif": False
                },
                ip_address=request.client.host if request.client else None
            )
