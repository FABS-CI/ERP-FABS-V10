...
export default function BonsRetour() {
  ...
  useEffect(() => { fetchBrs(); /* eslint-disable-next-line */ }, [debouncedQ, statut]);

  const fetchBrs = async () => {
    setLoading(true);
    try {
      const data = await getBonsRetour({ statut, q: search, limit: 100 });
      setBrs(data);
    } catch (e) {
    ...
    <DashboardLayout>
      <div className="space-y-6" data-testid="bons-retour-page">
        <PageHeader
          ...
          subtitle="Retours clients FABS-BR — génération automatique d'avoirs"
          pagePath="/bons-retour"
          actions={
            canWrite && (
              <Button onClick={() => setShowForm(true)} className="bg-blue-600 hover:bg-blue-700 text-white h-9" data-testid="btn-nouveau-br">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                ...
                <Input
                  ...
                  onKeyDown={(e) => e.key === "Enter" && fetchBrs()}
                />
              </div>
              <div>
                <Label>Statut</Label>
