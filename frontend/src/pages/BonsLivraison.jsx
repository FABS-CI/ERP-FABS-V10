...
export default function BonsLivraison() {
  ...
  useEffect(() => { fetchBls(); /* eslint-disable-next-line */ }, [debouncedQ, statut]);

  const fetchBls = async () => {
    setLoading(true);
    try {
      const data = await getBonsLivraison({ statut, q: search, limit: 100 });
      setBls(data);
    } catch (e) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                ...
                <Input
                  ...
                  onKeyDown={(e) => e.key === "Enter" && fetchBls()}
                />
              </div>
              <div>
                <Label>Statut</Label>
